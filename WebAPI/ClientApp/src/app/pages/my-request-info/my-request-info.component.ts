...
export class MyRequestInfoComponent implements OnInit {
  ...
  public request: AvailabilityRequestModelForCustomer = new AvailabilityRequestModelForCustomer(0, "", new Date(), new Date(),
    "", "", 0, "", "", 0, 0, [new ImageModel(0, "", "", 0)], new Date(), new Date());
    ...
    await this.requestService.getForCustomer(id)
      .then(
        (data) => {
          ...
          if (data != null)
            this.titleService.setTitle("Заявка от " + this.datetimeService.convertDateToUTS(this.request.startRent.getDate()) + "." + this.datetimeService.convertDateToUTS(this.request.startRent.getMonth() + 1)
              + "." + this.request.startRent.getFullYear() + " в " + this.datetimeService.convertDateToUTS(this.request.startRent.getHours()) + ":00");
        }
      )
      .catch(
        (error) => {
